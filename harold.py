...
def create_LSH_matrix(list_filenames):
    ...
    for i,filename in enumerate(list_filenames):
        ...
        for n in range(n_frames):
            ...
            X[i,] += np.mean(video.get_data(idx_frame),axis=(0,1))
    X = X/frame
    X = normalize(X,axis = 1)
    return X
def create_LSH_matrix2(list_filenames):
    N = len(list_filenames)
    frame = 2
    X = np.zeros((N,3))
    for i,filename in enumerate(list_filenames):
        ...
        for n in range(frame):
            idx_frame = int((n/(frame-1))*length_video)
            X[i,] += np.mean(video.get_data(idx_frame),axis=(0,1))
    X = X/frame
    X = normalize(X,axis = 1)
    return X
